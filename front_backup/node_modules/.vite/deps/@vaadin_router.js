...
function getRoutePath(route) {
  return resolvePath(route == null ? void 0 : route.path);
}
...
function updateChainForRoute(context, match) {
  ...
  if (route && !route.__synthetic) {
    ...
    (_a = context.chain) == null ? void 0 : _a.push(item);
  }
}
var _context, _root;
var Resolver = class {
  ...
  async resolve(pathnameOrContext) {
    ...
    async function next(resume = false, parent = ((_a) => (_a = matches == null ? void 0 : matches.value) == null ? void 0 : _a.route)(), prevResult) {
      var _a2, _b;
      const routeToSkip = prevResult === null ? (_a2 = matches == null ? void 0 : matches.value) == null ? void 0 : _a2.route : void 0;
      ...
      currentContext = {
        ...
        chain: (_b = currentContext.chain) == null ? void 0 : _b.slice()
      };
...
function cacheRoutes(routesByName, route, routes, cacheKeyProvider) {
  var _a;
  const name = route.name ?? (cacheKeyProvider == null ? void 0 : cacheKeyProvider(route));
  if (name) {
    if (routesByName.has(name)) {
      (_a = routesByName.get(name)) == null ? void 0 : _a.push(route);
    } else {
...
function createLocation({ chain = [], hash = "", params = {}, pathname = "", redirectFrom, resolver, search = "" }, route) {
  ...
  return {
    baseUrl: (resolver == null ? void 0 : resolver.baseUrl) ?? "",
    getUrl: (userParams = {}) => resolver ? getPathnameForRouter(
      compile(getRoutePath2(chain))({ ...params, ...userParams }),
      resolver
    ) : "",
    hash,
    params,
...
function amend(fn, obj, ...args) {
  return (result) => {
    ...
    return maybeCall(obj == null ? void 0 : obj[fn], obj, ...args);
  };
}
...
var Router = class extends resolver_default {
  ...
  constructor(outlet, options) {
    ...
    const baseHref = baseElement == null ? void 0 : baseElement.getAttribute("href");
    super([], {
...
doRender_fn = async function(context, shouldUpdateHistory) {
  ...
  try {
    ...
    if (renderId === __privateGet(this, _lastStartedRenderId)) {
      ...
      for (const child of ((_a = __privateGet(this, _outlet)) == null ? void 0 : _a.children) ?? []) {
        child.remove();
      }
...
runOnBeforeCallbacks_fn = async function(newContext) {
  ...
  if (previousChain.length) {
    for (let i = 0; i < Math.min(previousChain.length, newChain.length); newContext.__divergedChainIndex++, i++) {
      if (previousChain[i].route !== newChain[i].route || previousChain[i].path !== newChain[i].path && previousChain[i].element !== newChain[i].element || !__privateMethod(this, _Router_instances, isReusableElement_fn).call(this, previousChain[i].element, newChain[i].element)) {
        break;
      }
    }
    ...
    if (newContext.__skipAttach) {
      for (let i = newChain.length - 1; i >= 0; i--) {
        callbacks = __privateMethod(this, _Router_instances, runOnBeforeLeaveCallbacks_fn).call(this, callbacks, newContext, { prevent }, previousChain[i]);
      }
...
copyUnchangedElements_fn = function(context, previousContext) {
  ...
  for (let i = 0; i < (context.__divergedChainIndex ?? 0); i++) {
    const unchangedElement = (_a = previousContext == null ? void 0 : previousContext.chain) == null ? void 0 : _a[i].element;
    if (unchangedElement) {
...
addAppearingContent_fn = function(context, previousContext) {
  ...
  __privateSet(this, _disappearingContent, Array.from((deepestCommonParent == null ? void 0 : deepestCommonParent.children) ?? []).filter(
    // Only remove layout content that was added by router
    (e) => __privateGet(this, _addedByRouter).has(e) && // Do not remove the result element to avoid flickering
    e !== context.result
  ));
  let parentElement = deepestCommonParent;
  for (let i = context.__divergedChainIndex ?? 0; i < (((_a = context.chain) == null ? void 0 : _a.length) ?? 0); i++) {
    const elementToAdd = context.chain[i].element;
    if (elementToAdd) {
      parentElement == null ? void 0 : parentElement.appendChild(elementToAdd);
...
runOnAfterLeaveCallbacks_fn = function(currentContext, targetContext) {
  var _a;
  if (!(targetContext == null ? void 0 : targetContext.chain) || currentContext.__divergedChainIndex == null) {
    return;
  }
  for (let i = targetContext.chain.length - 1; i >= currentContext.__divergedChainIndex; i--) {
    ...
    try {
      ...
      if ((_a = __privateGet(this, _disappearingContent)) == null ? void 0 : _a.includes(currentComponent)) {
...
animateIfNeeded_fn = async function(context) {
  var _a, _b;
  const from = (_a = __privateGet(this, _disappearingContent)) == null ? void 0 : _a[0];
  const to = (_b = __privateGet(this, _appearingContent)) == null ? void 0 : _b[0];
  const promises = [];
  const { chain = [] } = context;
  let config;
